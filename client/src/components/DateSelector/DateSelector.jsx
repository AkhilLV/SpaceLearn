...
function DateSelector() {
  const { cardData, setSelectedDateId, setSelectedDate } = useContext(CardContext);
  ...
  return (
    <div className="date-selector" onClick={handleDateSelectorClick}>
      {cardData.cardDates.map((cardDate, i) => <div key={cardDate.cardDateId} className={`date ${i === 0 && "active"}`} data-date={cardDate.cardDate} data-dateid={cardDate.cardDateId}>{dateToWords(new Date(cardDate.cardDate))}</div>)}
    </div>
  );
}
